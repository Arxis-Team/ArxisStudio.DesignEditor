...
Подсказка рисуется отрезком с засечками, а не линией через холст: линия читается как «здесь проходит ось», а измерение зазора — как «вот столько». Кисть своя, `DesignEditor.SpacingBrush`. Выключается через `InteractionOptions.IsEqualSpacingEnabled`; работает на перетаскивании, при изменении размера интервалов нет.
