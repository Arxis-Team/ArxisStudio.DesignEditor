...
`PublicSurfaceTests` сверяет `GetExportedTypes()` со списком из 39 типов. Новый публичный тип роняет тест — его нужно либо внести в список осознанно, либо сделать `internal`. Отдельные тесты запрещают утечку машин состояний и деталей overlay.
