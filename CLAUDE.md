...
- **Уровень контейнера** — `DesignEditorItemState` (`States/States.cs`): `ItemIdleState`, `ItemDraggingState`, `ItemResizingState`. У этого класса есть `ReEnter(...)` — вызывается при возврате из вложенного состояния.
...
- `PART_InteractionOverlayLayer` — временные оверлеи на время действия пользователя (сейчас marquee; сюда же планируются snap lines, guides, preview)
...
Порядок перекрытия идёт тем же путём: `SetDesignZIndex` — третий и последний метод-шов рядом с `SetDesignPosition`/`SetDesignSize`. Изменения наследуют общий `DesignChange`, чтобы хост не разбирал типы.
...
- `DesignEditorInteractionOptions` — `ZoomStep`, `DragStartThreshold`, `ResizeMinSize`, `NudgeStep`, `LargeNudgeStep`, `IsSnapToGridEnabled`, `SnapStep`
...
`PublicSurfaceTests` сверяет `GetExportedTypes()` со списком из 38 типов. Новый публичный тип роняет тест — его нужно либо внести в список осознанно, либо сделать `internal`. Отдельные тесты запрещают утечку машин состояний и деталей overlay.

Машины состояний (`ArxisStudio.States.*`), `SelectionAdornerLayer`, `SelectionAdornerInfo`, `DesignSurface` и свойства `SecondarySelectionAdorners` — **internal**. Вместе с ними internal стали `CurrentState`, `PushState`, `PopState` на обоих контролах.
...
Тестов — 138, все зелёные; сборка с 0 предупреждений. Метод работы, который себя оправдал и которого стоит держаться: **сначала характеризующий тест или замер, потом реализация**, и обязательная проверка, что правка нагружена — временно откатить её и убедиться, что падают именно нужные тесты. Он несколько раз менял постановку задачи: вложенные контейнеры не давали выделения вовсе (а не «не тот scope»), проблема рамки была в латченном владельце (а не в обрезке), событий выделения было 22 за жест, а виртуализация оказалась не нужна вообще — 1.28 мс на кадр при 500 элементах.

Сделано по дорожной карте: сетка, контракт изменений под undo/redo, клавиатура, события выделения, z-order, фиксация публичной поверхности, привязка к сетке.
