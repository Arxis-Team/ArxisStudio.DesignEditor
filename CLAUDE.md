...
- **Контейнер, выбранный только через индексный слой, читается как «выбран его ребёнок по умолчанию».** `SelectedIndex = 0` даёт `Scope == NestedTarget` вместо `Container`. Чинится не там, где видно: неявный ответ `ResolveSelectionTargets` **держит** гарантию «одно событие на клик». Путь указателя пишет слои в два шага — сначала индексный, потом target, — и между ними оверлей пересобирается. Сегодня промежуточный снимок совпадает с итоговым (оба дают вложенный контрол), поэтому второе событие гасится сравнением. Стоит ответить «выбрана форма» — и один клик начинает поднимать два события: сначала `Container`, потом `NestedTarget`. Замерено. Значит, порядок такой: сначала перевести путь указателя на общее ядро записи, чтобы оба слоя менялись одной транзакцией, и только потом исправлять чтение.
...
Тестов — 271, все зелёные; сборка с 0 предупреждений. Метод работы, который себя оправдал и которого стоит держаться: **сначала характеризующий тест или замер, потом реализация**, и обязательная проверка, что правка нагружена — временно откатить её и убедиться, что падают именно нужные тесты. Он несколько раз менял постановку задачи: вложенные контейнеры не давали выделения вовсе (а не «не тот scope»), проблема рамки была в латченном владельце (а не в обрезке), событий выделения было 22 за жест, а виртуализация оказалась не нужна вообще — 1.28 мс на кадр при 500 элементах.
