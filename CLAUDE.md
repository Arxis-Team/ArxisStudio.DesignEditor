...
Тестов — 289, все зелёные; сборка с 0 предупреждений. Метод работы, который себя оправдал и которого стоит держаться: **сначала характеризующий тест или замер, потом реализация**, и обязательная проверка, что правка нагружена — временно откатить её и убедиться, что падают именно нужные тесты. Он несколько раз менял постановку задачи: вложенные контейнеры не давали выделения вовсе (а не «не тот scope»), проблема рамки была в латченном владельце (а не в обрезке), событий выделения было 22 за жест, а виртуализация оказалась не нужна вообще — 1.28 мс на кадр при 500 элементах.
