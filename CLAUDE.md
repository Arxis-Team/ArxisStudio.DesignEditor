...
Кисть отдельная (`DesignEditor.UserGuideBrush`, своя для Light и Dark), толщина общая с линиями выравнивания.
...
- **Настройка внешнего вида направляющих.** Сейчас снаружи задаются только цвет (`DesignEditor.SnapGuideBrush`, отдельно для Light и Dark) и толщина (`DesignEditor.SnapGuide.Thickness`, в пикселях устройства). Дальше — три уровня, по возрастанию цены:
  1. **Штрих.** `SnapGuideLayer.Render` собирает `new Pen(brush, thickness)` без `DashStyle` — пунктирную направляющую задать нечем. Правка на несколько строк плюс ресурс.
  2. **Разные виды линий разным цветом.** `DesignSnapGuide` несёт `Orientation`, но не несёт, каким выравниванием линия порождена, поэтому покрасить выравнивание по центру иначе, чем по краю, слою нечем. Нужно расширить внутреннюю модель и завести ключи по видам.
  3. **Замена слоя целиком.** `SnapGuideLayer` и свойство `SnapGuides` — `internal`, и внешняя сборка не может ни назвать тип в своём шаблоне, ни привязаться к свойству. Это фиксация публичного API: делать её стоит под конкретного потребителя с конкретным требованием, а не заранее. Сторожит `Snap_Guides_Are_Not_Public`.
