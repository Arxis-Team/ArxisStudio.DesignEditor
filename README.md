...
Первое живёт на редакторе, потому что линеек две, а выключатель нужен один; линейка следит за ним сама, как и за масштабом. Второе — на линейке: «шкала мешает читать макет» и «линейка не нужна вовсе» это разные требования.
...
1. Подсказки о равных интервалах между соседями.
2. Замена слоя направляющих целиком. Цвет, толщина, штрих и разный цвет по виду выравнивания уже настраиваются ресурсами; слой отрисовки пока `internal` — публиковать его имеет смысл под конкретное требование.
