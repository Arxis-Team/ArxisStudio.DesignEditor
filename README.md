...
![Общий вид редактора](docs/images/overview.png)

Пять экранов приложения умного дома лежат на бесконечном холсте: каждый — отдельный `DesignEditorItem` с загруженной разметкой. По краям линейки, сверху шапка: в верхнем ряду шаг сетки, переключатели показа, масштаб, команды вида и отмена, в нижнем — что выбрано и какие у него числа. Пользовательские направляющие хост задал, но показ их выключен — за него отвечает переключатель `guides`.
...
Пунктиром показаны линии выравнивания, найденные во время перетаскивания: розовые — по краям, фиолетовая — по центру. Оранжевый отрезок справа — подсказка о равном зазоре.
...
Карточку «Климат» ведут внутри формы, и оба зазора вокруг неё — до ряда кнопок сверху и до карточки снизу — показаны отрезками с засечками: это измерение, а не ещё одна ось. Каждый отрезок лежит по середине полосы, общей у элемента и того соседа, до которого он меряет, — поэтому у разных по ширине соседей они и оказываются в разных местах. Серым пунктиром на том же кадре видно выравнивание по соседям в ряду: оси независимы, и одна занята выравниванием, пока другая занята интервалом.
