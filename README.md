...
Пять экранов приложения умного дома лежат на бесконечном холсте: каждый — отдельный `DesignEditorItem` с загруженной разметкой. По краям линейки, сверху шапка: в верхнем ряду шаг сетки, переключатели показа, масштаб, команды вида и отмена, в нижнем — что выбрано и какие у него числа. Слева панель групп — она построена целиком на публичном API и показывает, что редактор о группах отвечает. Пользовательские направляющие хост задал, но показ их выключен — за него отвечает переключатель `guides`.
