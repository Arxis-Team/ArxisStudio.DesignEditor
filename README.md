...
- `DesignEditor.ApplyGeometry(target, bounds)` — применить геометрию, не создавая новой записи
...
`ApplyGeometry` не поднимает `EditCompleted`, поэтому отмена не возвращается в стек и не зацикливается.
