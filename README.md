...
Пять экранов приложения умного дома лежат на бесконечном холсте: каждый — отдельный `DesignEditorItem` с загруженной разметкой. По краям линейки, сверху шапка с шагом сетки, переключателями показа и масштабом. Синие линии — пользовательские направляющие, заданные хостом.
...
Оба зазора вокруг перетаскиваемого экрана показаны отрезками с засечками: это измерение, а не ещё одна ось. Каждый отрезок лежит по середине полосы, общей у элемента и того соседа, до которого он меряет, — поэтому у разных по высоте соседей они и оказываются на разной высоте.
